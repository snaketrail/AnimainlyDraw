...
- **Two model packagings** — a single-file checkpoint (SD, SDXL, Pony) loaded with `CheckpointLoaderSimple`, or a split model whose UNet, text encoder and VAE are three separate files. Newer models ship split and cannot be loaded as a checkpoint at all, so the app offers both and lists whatever ComfyUI can actually see. Known models bring their own published sampler settings — pick [Anima](https://huggingface.co/circlestone-labs/Anima) and it switches to the 30 steps and CFG 4 its own workflow specifies, because the SDXL defaults give visibly worse output on it.
