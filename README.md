...
- **Speech bubbles** — speech, thought, shout, whisper, caption and trail dots. They sit above the panels, so one can cross a border or sit in a gutter, and are constrained to the printable page.
...
Screentones and halftone fills, speed lines, panel border styles, and a reader mode.

### AI integration (later)

A toolbar section for connecting local AI services, so the app can generate art rather than
only import it:

- **Stable Diffusion** — generate characters, backgrounds and props straight into the library.
- **LM Studio** — a local LLM for writing the story.

The end goal is an agent that drafts a story, generates the art for it, and lays the pages
out. This is deliberately last: it only makes sense once the editor underneath it is solid,
and both services are reached over a local HTTP API, so no keys leave the machine.
